...
export const socketService = {
  createRoom: (socket: Socket, playerName: string, gameMode: GameMode, presetCategory?: PresetCategory) => {
  ...
  assignCharacter: (socket: Socket, roomId: string, targetPlayerId: string, character: string) => {
    socket.emit('assign-character', roomId, targetPlayerId, character);
  },

  endRound: (socket: Socket, roomId: string, callback?: (response: { success?: boolean; error?: string }) => void) => {
